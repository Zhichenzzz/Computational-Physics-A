...
#define NUM_PARTICLE 10000                   //总共生成的点数
#define NUM_WALKING 2000                     //每次行走的粒子数
...
bool if_collision(Particle P1, const Particle& P2) {
...
void Circle_particle(Particle *P)
{
    z=Sch::Sch_16807(z);
    double ran=1.*z/M;
    P->x=int(circle* cos(2*PI*ran));         //格点直接舍入float为int存入x,y坐标
    P->y=int(circle* sin(2*PI*ran));
}
//正方边界
...
{
    if (P->length()>1.05*circle)                //如果粒子运动到1.05r以外，则重新从边界出发设置
        Circle_particle(P);
}
//聚集模型
void if_Diffusion()
...
void Change_Circle()
{
    if (now_unfree%1000==0&&now_unfree<=1000&&now_unfree>0)      //聚集粒子数为500,1000时更新半径
    {
        circle=circle+50;
        Set_initial();
    }
//    if (now_unfree%5000==0&&now_unfree<10000&&now_unfree>=1000)      //每变成1000的倍数更新圆半径
//    {
//        circle=circle+15;
//        Set_initial();
//    }
    if (now_unfree%5000==0&&now_unfree<10000&&now_unfree>=1000)      //每变成5000的倍数更新圆半径
    {
        circle=circle+150;
        Set_initial();
    }
